...
class DropoutNet(NeuralNet):
    ...
    def __init__(self, numpy_rng, theano_rng=None,
                 ...
                 layers_sizes=[8000, 8000, 8000, 8000],
        ...
        for layer, layer_type, n_in, n_out, dr in zip(self.layers,
            ...
            if fast_drop:
                this_layer.output = fast_dropout(numpy_rng, this_layer.output)
            else:
...
def add_fit_score_predict(class_to_chg):
    ...
    def predict(self, x):
        batch_x = T.fmatrix('batch_x')
        compute_y = theano.function(inputs=[theano.Param(batch_x)],
